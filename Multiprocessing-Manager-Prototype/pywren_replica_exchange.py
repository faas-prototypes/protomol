    ...
    while num_replicas_completed < len(replica_list):
        ...
        for j in replicas_to_run:
            if not replica_list[j].running:
                ...
                task_dictionary['distributed_map'] = shared_map
...
def serverless_task_process(task,distributed_map):
    # get input files
    ...
    res = distributed_map[input_local_file_pdb]
    ...
    res = distributed_map[input_par_file]
    ...
    res = distributed_map[input_psf_file]
    ...
    if (task.input_local_file_velocity is not None):
        ...
        res = distributed_map[input_vel_file]
    ...
    for conf_entry in task.input_conf_file:
        ...
        res = distributed_map[remote_config]
    ...
    with open(temp_dir + '/' + output_file_remote_velocity, 'rb') as sourceFile:
        distributed_map[protomol_utils.remove_first_dots(output_file_local_velocity)] = sourceFile.readlines()
    ...
    with open(temp_dir + '/' + output_file_pdb_name, 'rb') as sourceFile:
        distributed_map[ protomol_utils.remove_first_dots(output_file_pdb)] = sourceFile.readlines()
