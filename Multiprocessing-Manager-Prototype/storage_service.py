import shared_dict as distributedMap
...
def get_file(file_key):
    return distributedMap.get_value(file_key)
...
def save_protomol_template(output_path, pdb_file, psf_file, par_file, monte_carlo_step, md_steps, output_freq,
    ...
    distributedMap.save_file_to_redis(cfg_file_name, serialized_object)

    return cfg_file_name

def get_protomol_template_as_file(key):

    protomol_template_file = OrderedDict(json.loads(distributedMap.get_value(key)))
    write_str = build_file(protomol_template_file)
    return write_str

def upload_binary_file(src, target_key):
    with open(src, 'rb') as f:
        distributedMap.save_file_to_redis(target_key,f.read())

def upload_file(src, target_key):
   distributedMap.save_file_to_redis(target_key,src)
